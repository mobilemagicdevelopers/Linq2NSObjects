...
xcodebuild -target "$TARGET" -sdk iphoneos8.1 "ARCHS=armv6 armv7 armv7s arm64" -configuration "Release" clean build > /dev/null
xcodebuild -target "$TARGET" -sdk iphonesimulator8.1 "ARCHS=i386 x86_64" "VALID_ARCHS=i386 x86_64" -configuration "Release" clean build > /dev/null
